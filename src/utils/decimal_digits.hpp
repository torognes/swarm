...
#ifndef SWARM_UTILS_DECIMAL_DIGITS_H
...
//
// This is the shared half of what replaces swarm's integer std::fprintf
// calls. It is deliberately sink-agnostic -- it hands back a View<char> over
// the digits and knows nothing about where they go. print_number.hpp adds
// the std::FILE * sink on top; a caller appending to a std::string (the
// CIGAR builder in cigar.cpp) uses this header directly and keeps <cstdio>
// out of its include list, the same split print_view.hpp and view_stream.hpp
// already make for View<char>.
//
// Why not std::to_string: before libstdc++ 11 its integral overloads are
// __gnu_cxx::__to_xstring(&std::vsnprintf, "%ld", ...) -- the run-time
// format string this exists to remove -- and swarm still supports GCC 4.9.
// The digits are also produced without the temporary std::string.
//
...
namespace decimal {
  ...
  // digits10 is the number of decimal digits that survive a round trip,
  // one fewer than the widest representation, hence the + 1.
  ...
  // returned by to_decimal() borrows it: the view must not outlive the
  // buffer it was filled from. Every caller in swarm consumes the view in
  // the same statement or the next one.
  using Buffer = std::array<char, max_width>;


  namespace detail {

    // A value split into the parts the digit loop needs. Kept together so
    // that one dispatch answers both questions.
    struct Signed_magnitude {
      uint64_t magnitude;
      bool     negative;
    };

    // Tag dispatch on signedness rather than a plain 'if (value < 0)': for
    // an unsigned Integer that comparison is always false, which GCC
    // reports under -Wtype-limits, and swarm's DEBUG build enables it.
    ...
    constexpr auto split_sign(Integer const value, std::true_type /* is_signed */) -> Signed_magnitude {
      ...
      // type. Unsigned arithmetic is modulo 2^64, so 0 - static_cast<
      // uint64_t>(value) is the magnitude for every negative input, the
      // minimum included -- that is the case a hand-written -value misses.
      return value < 0
  ...
  //
  // Written back to front, because that is the order repeated division
  // yields the digits in; the view therefore starts somewhere inside the
  // buffer rather than at its beginning, which is why the width is returned
  // with the pointer instead of being re-derived by the caller.
  //
  ...
  auto to_decimal(Buffer & buffer, Integer const value) noexcept -> View<char> {
    ...
    // The buffer cannot be overrun: max_width above is the widest decimal
    // form of any type the static_asserts admit, sign included, and the loop
    // writes exactly one character per decimal digit. This states that
    // invariant rather than enforcing it -- by the time it could fail the
    // writes have already happened -- and is here as a tripwire for a future
    // change to max_width or to the accepted types.
...
// tests:
//
// Checked against snprintf as the reference, 74 cases, under
// -fsanitize=undefined,address -D_GLIBCXX_DEBUG and the DEBUG warning set,
// and instantiated for the four target toolchains. The cases below are the
// ones a hand-rolled digit loop gets wrong.
//
// #include <cassert>
// #include <cstdint>
// #include <string>
...
#endif  // SWARM_UTILS_DECIMAL_DIGITS_H
